...
#grafico 7 -distribuicao de precos por variedade(violinplot)
...
ax_violino_preco_var = plt.gca()

sns.violinplot(
    ax=ax_violino_preco_var, x='variety', y='price', data=df_plot_violino_variedade,
    order=top_variedades_nomes, palette=[bordo_profundo]*10, linewidth=1.2, inner='point', cut=0, density_norm='width'
)

ax_violino_preco_var.set_ylim(0, df_plot_violino_variedade['price'].quantile(0.85))
ax_violino_preco_var.set_title('DISTRIBUIÇÃO DE PREÇOS PELAS TOP 10 VARIEDADES', fontsize=13, fontweight=peso_fonte_titulo, color=preto_falso)
