import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import numpy as np
import matplotlib.ticker as mtick
from matplotlib.colors import LinearSegmentedColormap
import re
import os



diretorio_script = os.path.dirname(os.path.abspath(__file__))

#juntando o diretorio do escript com o nome do arqv
arqv = os.path.join(diretorio_script, 'arquivo.csv')

#verificand se o arquivo existe antes de tentar abrir
if not os.path.exists(arqv):
    arqv = os.path.join(os.path.dirname(diretorio_script), 'arquivo.csv')
    
if not os.path.exists(arqv):
    print(f"ERRO CRÍTICO: Não encontrei 'arquivo.csv'.\nO script está procurando em: {arqv}")
    exit()

df_tabela = pd.read_csv(arqv)

if 'Unnamed' in df_tabela.columns:
    df_tabela = df_tabela.drop('Unnamed: 0', axis=1)

#limpeza geral basica
colunas_nao_nan = ['country', 'province', 'variety', 'price', 'points']
df_tabela.dropna(subset=colunas_nao_nan, inplace=True)

#definicao das cores personalizadas
preto_falso = '#361A20'
bordo_profundo = "#6B2F3D"
azul_moderno = '#46647E'
areia_quente = '#D4BFAA'
verde_oliva = '#2E8B57'
rosa_queimado ='#C08081'
cinza_elegante = '#5A5A5A'
ouro_antigo = '#C2B280'
branco = "#FFFFFF"
areia_quente_fundo = '#F9F9F9'

#config de fontes
fonte_titulos_familia = 'serif'
fonte_corpo_familia = 'sans-serif'
peso_fonte_titulo = 'heavy'
cor_linhas_grade_sutil = "#E0E0E0"

sns.set_theme(style="whitegrid")


# GRAFICO 16:swarmplot(top 5 países)


df_tabela.rename(columns={'points': 'PONTOS'}, inplace=True)


top_5_paises_swarm = df_tabela.groupby('country')['PONTOS'].mean().nlargest(5).index
df_swarm_raw = df_tabela[df_tabela['country'].isin(top_5_paises_swarm)]

#fzd mostragem de 60 vinhos por país
df_swarm_sample = df_swarm_raw.groupby('country').apply(lambda x: x.sample(min(len(x), 60), random_state=1)).reset_index(drop=True)

fig, axes = plt.subplots(1, 2, figsize=(16, 6.5))
fig.patch.set_facecolor(areia_quente_fundo)
paleta_swarm = sns.color_palette([bordo_profundo, ouro_antigo, verde_oliva, azul_moderno, rosa_queimado], n_colors=5)

# plot pontos
sns.swarmplot(
    ax=axes[0], 
    x='country', 
    y='PONTOS', 
    data=df_swarm_sample, 
    order=top_5_paises_swarm, 
    palette=paleta_swarm, 
    size=3.5
)
axes[0].set_title('DISTRIBUIÇÃO DE PONTOS', fontweight='bold', color=preto_falso)
sns.despine(ax=axes[0])

# plot preco
sns.swarmplot(
    ax=axes[1], 
    x='country', 
    y='price', 
    data=df_swarm_sample, 
    order=top_5_paises_swarm, 
    palette=paleta_swarm, 
    size=3.5
)
axes[1].set_title('DISTRIBUIÇÃO DE PREÇOS', fontweight='bold', color=preto_falso)
sns.despine(ax=axes[1])

plt.tight_layout()
plt.show()