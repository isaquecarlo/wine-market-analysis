...
#GRAFICO 14:tamanho da descricao vs ptos

df_tabela['TAMANHO DA DESCRIÇÃO'] = df_tabela['description'].str.len()

if 'PONTOS' not in df_tabela.columns:
    df_tabela.rename(columns={'points': 'PONTOS'}, inplace=True)

df_plot_desc = df_tabela[df_tabela['TAMANHO DA DESCRIÇÃO'] <= 600].sample(n=700, random_state=42)

plt.figure(figsize=(10, 6.5))
sns.scatterplot(
    x='TAMANHO DA DESCRIÇÃO', 
    y='PONTOS', 
    data=df_plot_desc, 
...
sns.regplot(
    ...
    data=df_plot_desc, 
    scatter=False, 
    line_kws={'color': preto_falso, 'linewidth': 2}
)

plt.title('TAMANHO DA DESCRIÇÃO VS. PONTUAÇÃO', fontsize=15, fontfamily=fonte_titulos_familia, fontweight=peso_fonte_titulo, color=preto_falso)
plt.xlim(0, 600)
sns.despine()
plt.tight_layout()
plt.show()